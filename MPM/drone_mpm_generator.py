...
def compute_vector(cur, nxt, same_count, result_l, result, zeros, z_value, sigma, direction='peak'):
    ...
    img_lm[min(nxt[1]-1, 1079), min(nxt[0]-1, 1919)] = 255
...
@hydra.main(config_path='config/drone_mpm_generator.yaml')
def main(cfg):
    # save_root = to_absolute_path(cfg.path.save_path)
    save_root = cfg.path.save_path
    print(save_root)
    if not os.path.exists(save_root):
        os.makedirs(save_root)
    img_path_list = os.listdir(cfg.file.img_path)
    # print(img_path_list)
    seq_num =int(len(img_path_list)/300)
    print(seq_num)
    img1_path_list = []
    txt_path_list = []
    for i in range(seq_num):
        img1_path_list.append(img_path_list[i*300])
        txt_path_list.append('00'+img_path_list[i*300][3:6]+'.txt')
    print(img1_path_list)
    print(txt_path_list)
    for th in range(6, 7):
        print(txt_path_list[th])
        if not os.path.exists(os.path.join(save_root, txt_path_list[th].split('.')[0])):
            os.mkdir(os.path.join(save_root, txt_path_list[th].split('.')[0]))
        track_let = np.loadtxt(os.path.join(cfg.file.txt_path, txt_path_list[th])).astype('int')
        # print(track_let)
        image_size = cv2.imread(os.path.join(cfg.file.img_path, img1_path_list[th])).shape
        save_path = os.path.join(save_root, txt_path_list[th].split('.')[0])
