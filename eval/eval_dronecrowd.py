...
def load_groundtruth(list_path, gt_path, num_imgs=300, default_iou_thr=25):
...
def load_tracker_results(list_path, res_path, trackName):
    ...
    for seq_id in all_sequences:
        ...
        img_ids_seq = res[:, 0].astype(int)      
        track_ids_seq = res[:, 1].astype(int)   
        bboxes_xywh = res[:, 2:6]                
        confs_seq = res[:, 6]                   
...
def calc_ap_with_log(res_path, trackName,track_confs, tp_cell, fp_cell, num_track_per_class, defaultTrackThr, log_path="ap_log.txt"):
    ...
    with open(log_path, "a", encoding="utf-8") as f:
        ...
        mean_ap = np.mean(aps)
        f.write(f"---{res_path}-->>{trackName}---1.8\n")
...
if __name__ == "__main__":
    ...
    list_path = "test_list/test_total.txt"
    gt_path = " DroneCrowd/annotations" #ground_truth
    
  
    res_path = "test_thd/frame_8_16_13_0.4_dmax15_dis7"
   
    trackName = "diffusion_clip_dis_frame8_16_13_0.4_dmax15_dis7" 
    log_path = "/thd/ap_log.txt"
