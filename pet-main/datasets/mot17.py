...
class mot17(Dataset):
    ...
    def __init__(self, data_root, transform=None, train=False, flip=False):
        ...
        sequences = [seq for seq in os.listdir(f"{data_root}/{prefix}") if seq.endswith("DPM")]
        cls_people = {1,7}
    ...
    def __getitem__(self, index):
        ...
        img, points = load_data_mot17((img_path, gt_info), train=self.train)
        ...
        if random.random() > 0.5 and self.train and self.flip:
            ...
            points[:, 1] = self.patch_size - points[:, 1]  # 更新 x 坐标
...
def load_data_mot17(img_gt_tuple, train=False):
...
def build(image_set, args):
    ...
    if image_set == 'train':
        train_set = mot17(data_root, train=True, transform=transform, flip=True)
        return train_set
    elif image_set == 'val':
        val_set = mot17(data_root, train=False, transform=transform)
        return val_set
...
def visualize_points_on_images(dataset, num_samples=5, save_dir="./vis_samples"):
    ...
    for idx in indices:
        ...
        img, points = load_data_mot17((img_path, gt_infos), train=False)
...
# # 1.load MOT20
# dataset = mot17(data_root="/home/data_SSD/zk/MOT17", train=True, flip=False)
...
# output_dir = "/home/zk/charac_pos/PET-main/dataset_visual"

# save_dir = os.path.join(output_dir,"split0.2_train_mot17_vis_2_bbox_dy30_1_7")
# # print("train set,valid set",len(dataset_train),len(dataset_val))
