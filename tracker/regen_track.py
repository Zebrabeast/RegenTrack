...
def fuse_appearance_motion_scores(appearance_scores, motion_scores, modified_scores,distance_threshold=15,max_distance=15.0):
    ...
    alpha = 0.2 + ((distance_est / max_distance) * 0.4)
    appearance_weights = np.clip(alpha, 0.2, 0.6)
...
if __name__ == '__main__':
    ...
    # over_seq = ["00070"]
    root_dir = arg.dataset_path#"dataset/test_data" #  image path
   
    sequence_dirs = sorted([
        os.path.join(root_dir, d) for d in os.listdir(root_dir)
        if os.path.isdir(os.path.join(root_dir, d))
    ])
    output_dir = arg.output_path #output path
